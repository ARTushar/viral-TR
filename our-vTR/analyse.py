...
def main(argv=None):
    ...
    model.load_weights('model_weights/w1.h5')
    ...
    with open('kernel_weights/1', 'w') as f:
        for layer_num in range(2, 6):
            ...
            for i in range(npa.shape[0]):
                ...
                # print(df.head())
                df.to_csv('kernel_weights/1.csv', mode='a', sep='\t', float_format='%.3f')
    ...
    #             # plt.show()
    #             plt.savefig('logos/l1/logo' + str(layer_num) + '_' + str(i) + '.png', dpi=50)
